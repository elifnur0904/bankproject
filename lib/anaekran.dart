...
import 'package:starbankproject/kriptoekrani.dart';
...
class anaSayfaState extends State<anaSayfa> {
  ...
  Future<void> paylasIBAN(String IBANPar, String surnamePar) async {
    //Kullanıcı ibanını ve adını paylaşma yapabilen method.
    try {
      await Share.share(IBANPar + "\n" + surnamePar);
    } catch (e) {
      print(e);
    }
  }
  ...
  Map<String, IconData> fiyatIconList = {};
  Future<void> kriptoGetirMethod() async {
    try {
          ...
          for (var item in data) {
            ...
            if (oncekiFiyatList.containsKey(symbol)) {
              ...
              if (currentPrice > previousPrice) {
                fiyatIconList[symbol] = Icons.arrow_upward; // Yükseldi
              } else if (currentPrice < previousPrice) {
                fiyatIconList[symbol] = Icons.arrow_downward; // Düştü
              } else {
                fiyatIconList[symbol] = Icons.remove; // Değişmedi
              }
            } else {
              fiyatIconList[symbol] =
  ...
  @override
  void initState() {
    //Sayfa ilk yüklendiği anda çalışan methoddur.
    super.initState();
    kriptoGetirMethod(); //Kr,ipto verilerini getiren method.
    timer = Timer.periodic(const Duration(seconds: 2), (Timer t) {
      kriptoGetirMethod();
    });
    kullaniciBilgiKontrol(); //Kullanıcı bilgilerini getiren method

    kartBilgileriGetirMethod(); //Kullanıcının kart bilgilerini getiren method.
  }

  CollectionReference gecmisDocument =
      FirebaseFirestore.instance.collection('accountTransactionHistory');
  String userNameSurname = "";
  Future<void> kullaniciBilgiKontrol() {
  ...
  void guncelleKriptoMethod(
    ...
    try {
      ...
      deleteDataforeign(deleteDocID).whenComplete(() {});
    } catch (e) {
      print("Failed to update data: $e");
    }
  }

  kontrolveGuncelleMethod(String cardNumber, String newBalancecalc) async {
    //Para değerini güncelleyen method.
    ...
    if (querySnapshot.docs.isNotEmpty) {
      ...
      Fluttertoast.showToast(
          ...
          fontSize: 16.0);
      kartBilgileriGetirMethod().whenComplete(() {
  ...
  var uuid = Uuid();
  Future<void> kartKontrolGuncelleMethod(
      String cardNumber,
      String newBalanceSent,
      String newBalanceSender,
      String docIDSender) async {
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance
        .collection('userCreditCards')
        .where('userCreditCardNumber', isEqualTo: cardNumber)
        .get();
    ...
    if (querySnapshot.docs.isNotEmpty) {
      for (var doc in querySnapshot.docs) {
        ...
        try {
          double newBalancecalc = double.parse(doc.get('userCreditBalance')) +
              double.parse(newBalanceSent);
          await documentRef.update({
            'userCreditBalance': newBalancecalc.toString(),
          }).whenComplete(() async {
            ...
            try {
              await documentRefR.update({
                ...
                DocumentReference gecmisDocumentdoc =
                    gecmisDocument.doc(generateID);
                DateTime now = DateTime.now();
                String formattedDate =
                    DateFormat('kk:mm:ss \n EEE d MMM').format(now);

                await gecmisDocumentdoc.set({
                  ...
                  String generateID = uuid.v4();
                  gecmisDocumentdoc = gecmisDocument.doc(generateID);
                  await gecmisDocumentdoc.set({
                    'amount': txtMoneyController.text,
  ...
  Future<void> kartBilgileriGetirMethod() {
  ...
  Future<void> deleteDataforeign(String docId) async {
    //Döviz satma methodu.
    try {
      await FirebaseFirestore.instance
                        ...
                        return Column(
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              child: ElevatedButton.icon(
                                onPressed: () {
                                  double testd = double.parse("4650.555");
                                  print("****: " + testd.toString());
                                   if (txtIBANController.text != "" ||
                                      txtMoneyController.text != "" ||
                                      txtExplaneController.text != "") {
                                    double accountMoney = double.parse(
                                        userCreditBalanceList[
                                            userCreditCardNumberList.indexOf(
                                                _selectedItem.toString())]);
                                    double enteredMoney =
                                        double.parse(txtMoneyController.text);
                  ...
                  child: Container(
                    ...
                    child: Container(
                      ...
                      child: ListView.builder(
                        ...
                        itemCount: userCreditBalanceList.length,
                          ...
                          return Container(
                            ...
                            child: Stack(
                              children: [
                                Positioned(
                                    child: CreditCardUi(
                                  cardHolderFullName: userNameList[index],
                                  cardNumber: userCreditCardNumberList[index],
                                  validThru: userCreditLastDateList[index],
                                  ...
                                  balance: double.parse(
                                      userCreditBalanceList[index].toString()),
                                  showValidFrom: false,
                                  creditCardType: CreditCardType.visa,
                                  autoHideBalance: false,
                                  enableFlipping: true,
                                  cvvNumber: userCreditCvvNumberList[index],
                                )),
                                Positioned(
                                    top: 10,
                                    right: 10,
                                    child: Row(
                                      children: [
                                        IconButton(
                                          ...
                                          onPressed: () {
                                            paylasIBAN(
                                                userCreditCardNumberList[index],
                                                userNameList[index]);
                                          },
                                        )
                                      ],
                                    ...
                                    children: [
                                      IconButton(
                                        icon: Icon(
                                          ...
                                          size: 19,
                                          color:
                                              Color.fromARGB(240, 141, 32, 24),
                                        ),
                                        onPressed: () {
                                          Dialogs.materialDialog(
                                              msg:
                                                  'Kayıtlı kartınızı kaldırmak istediğinizden emin misiniz?',
                                              title: "Kartı Kaldır",
                                              color: Colors.white,
                                              context: context,
                                              actions: [
                            ...
                            children: [
                              ...
                              IconButton(
                                ...
                                onPressed: () {
                                  Dialogs.materialDialog(
                                      msg:
                                          'Yeni bir kredi kartı eklemek ister misiniz?',
                                      title: "Kart Ekle",
                                      color: Colors.white,
                                      context: context,
                                      actions: [
              ...
              child: Row(
                ...
                children: [
                  Text(
                    "Kripto",
                    style: GoogleFonts.kanit(
                        fontSize: 19, color: Color.fromARGB(255, 5, 15, 56)),
                  ),
                  InkWell(
                    onTap: () {
                      Navigator.push(
              ...
              child: kriptoFiyatList.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final IconData icon =
                            fiyatIconList[symbol] ?? Icons.remove;
...
///////////////////////
                                                                          ...
                                                                          await userRefdocForegn.set({
                                                                            ...
                                                                            'foreignTotalAmount':
                                                                              ...
                                                                              {
                                                                                kontrolveGuncelleMethod(_selectedItem!, newAmout.toString())
                                                                              });
